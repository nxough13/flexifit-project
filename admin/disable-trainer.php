...
try {
    ...
    $stmt->bind_param("i", $trainerId);
    if (!$stmt->execute()) {
        throw new Exception("Failed to disable trainer");
    }
    $stmt->close();
    ...
    try {
        ...
        $mail->isHTML(true);
        $mail->Subject = "Your Trainer Account Has Been Disabled";
        $mail->Body = "
            <h2>Account Disabled</h2>
            <p>Dear {$trainer['first_name']},</p>
            <p>Your trainer account has been disabled by the FlexiFit admin team.</p>
    ...
    error_log("Error in disable-trainer.php: " . $e->getMessage());
}
